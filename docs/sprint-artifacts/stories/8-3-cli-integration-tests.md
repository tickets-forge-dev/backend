...
4. `forge tickets` non-TTY path is covered by `src/commands/__tests__/list.test.ts` with tests for: (a) not-logged-in exits 1; (b) non-TTY: no tickets → prints empty message and exits 0; (c) non-TTY: tickets present → prints tab-separated rows and exits 0; (d) `--all` flag passes `all=true` query param instead of `assignedToMe=true`; (e) API error exits 2.
