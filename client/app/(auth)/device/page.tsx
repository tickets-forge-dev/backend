'use client';

import { useState, Suspense } from 'react';
import { useSearchParams } from 'next/navigation';
import { Button } from '@/core/components/ui/button';
import { Input } from '@/core/components/ui/input';
import { useAuthStore } from '@/stores/auth.store';
import { auth } from '@/lib/firebase';

function GoogleIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none">
      <path
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
        fill="#4285F4"
      />
      <path
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        fill="#34A853"
      />
      <path
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
        fill="#FBBC05"
      />
      <path
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        fill="#EA4335"
      />
    </svg>
  );
}

function GitHubIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor">
      <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
    </svg>
  );
}

type Status = 'idle' | 'submitting' | 'success' | 'error';

function DevicePageContent() {
  const searchParams = useSearchParams();
  const { user, signInWithGoogle, signInWithGitHub, isLoading } = useAuthStore();

  const [code, setCode] = useState(searchParams.get('code') ?? '');
  const [status, setStatus] = useState<Status>('idle');
  const [errorMessage, setErrorMessage] = useState('');

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user || !code.trim() || !auth?.currentUser) return;

    setStatus('submitting');
    setErrorMessage('');

    try {
      const idToken = await auth.currentUser.getIdToken();
      const apiUrl =
        process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000/api';

      const res = await fetch(`${apiUrl}/auth/device/verify`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${idToken}`,
        },
        body: JSON.stringify({ userCode: code.trim().toUpperCase() }),
      });

      if (res.ok) {
        setStatus('success');
      } else {
        const body = (await res.json()) as {
          error?: string;
          message?: string;
        };
        const msg = body.error ?? body.message ?? 'Invalid code. Try again.';
        setErrorMessage(
          msg === 'invalid_code'
            ? 'Code not found or already used. Check your terminal and try again.'
            : msg === 'expired_token'
              ? 'This code has expired. Run `forge login` again to get a new one.'
              : msg,
        );
        setStatus('error');
      }
    } catch {
      setErrorMessage('Network error. Please try again.');
      setStatus('error');
    }
  };

  const handleCodeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    // Auto-format: insert dash after 4 chars
    let val = e.target.value.toUpperCase().replace(/[^A-Z2-9]/g, '');
    if (val.length > 4) {
      val = val.slice(0, 4) + '-' + val.slice(4, 8);
    }
    setCode(val);
    if (status === 'error') {
      setStatus('idle');
      setErrorMessage('');
    }
  };

  // Step 1: Sign in
  if (!user) {
    return (
      <div className="space-y-10">
        <div>
          <h2 className="text-[var(--text-xl)] font-semibold text-white">
            Sign in to authorize CLI
          </h2>
          <p className="text-[var(--text-sm)] text-[#a1a1aa] mt-1">
            Sign in with your Forge account to link your terminal session
          </p>
        </div>

        <div className="space-y-3">
          <Button
            onClick={() => signInWithGoogle()}
            disabled={isLoading}
            variant="outline"
            className="w-full h-11 justify-center gap-3 text-white font-medium border-[#27272a] bg-[#18181b] hover:bg-[#27272a] hover:border-[#3f3f46]"
            size="lg"
          >
            <GoogleIcon className="w-5 h-5 shrink-0" />
            {isLoading ? 'Signing in...' : 'Continue with Google'}
          </Button>

          <Button
            onClick={() => signInWithGitHub()}
            disabled={isLoading}
            variant="outline"
            className="w-full h-11 justify-center gap-3 text-white font-medium border-[#27272a] bg-[#18181b] hover:bg-[#27272a] hover:border-[#3f3f46]"
            size="lg"
          >
            <GitHubIcon className="w-5 h-5 shrink-0" />
            Continue with GitHub
          </Button>
        </div>
      </div>
    );
  }

  // Step 3: Success
  if (status === 'success') {
    return (
      <div className="space-y-8 text-center">
        <div className="flex justify-center">
          <div className="w-16 h-16 rounded-full bg-green-500/10 flex items-center justify-center">
            <svg
              className="w-8 h-8 text-green-400"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M5 13l4 4L19 7"
              />
            </svg>
          </div>
        </div>
        <div>
          <h2 className="text-[var(--text-xl)] font-semibold text-white">
            CLI authorized
          </h2>
          <p className="text-[var(--text-sm)] text-[#a1a1aa] mt-2">
            Return to your terminal — authentication is complete.
          </p>
        </div>
      </div>
    );
  }

  // Step 2: Enter code
  return (
    <div className="space-y-10">
      <div>
        <h2 className="text-[var(--text-xl)] font-semibold text-white">
          Authorize CLI
        </h2>
        <p className="text-[var(--text-sm)] text-[#a1a1aa] mt-1">
          Enter the code shown in your terminal
        </p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-4">
        <Input
          value={code}
          onChange={handleCodeChange}
          placeholder="XXXX-XXXX"
          className="text-center text-2xl font-mono tracking-[0.3em] h-14 bg-[#18181b] border-[#27272a] text-white placeholder:text-[#52525b] focus:border-[#7c3aed]"
          maxLength={9}
          autoFocus
          autoComplete="off"
          autoCorrect="off"
          spellCheck={false}
        />

        {status === 'error' && (
          <p className="text-[var(--text-sm)] text-red-400 text-center">
            {errorMessage}
          </p>
        )}

        <Button
          type="submit"
          disabled={status === 'submitting' || code.length < 9}
          className="w-full h-11 bg-[#7c3aed] hover:bg-[#6d28d9] text-white font-medium"
        >
          {status === 'submitting' ? 'Authorizing...' : 'Authorize'}
        </Button>
      </form>

      <p className="text-center text-[var(--text-xs)] text-[#52525b]">
        Signed in as{' '}
        <span className="text-[#a1a1aa]">{user.email}</span>
      </p>
    </div>
  );
}

export default function DevicePage() {
  return (
    <Suspense>
      <DevicePageContent />
    </Suspense>
  );
}
