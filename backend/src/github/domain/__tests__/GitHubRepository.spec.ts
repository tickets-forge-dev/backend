...
describe('GitHubRepository', () => {
  ...
  describe('create', () => {
    ...
    it('should reject invalid repository ID', () => {
      expect(() => GitHubRepository.create({ ...validProps, id: 0 })).toThrow(
        'Repository ID must be a positive number',
      );

      expect(() => GitHubRepository.create({ ...validProps, id: -1 })).toThrow(
        'Repository ID must be a positive number',
      );
    });

    it('should reject empty full name', () => {
      expect(() => GitHubRepository.create({ ...validProps, fullName: '' })).toThrow(
    ...
    it('should reject empty name', () => {
      expect(() => GitHubRepository.create({ ...validProps, name: '' })).toThrow(
        'Repository name is required',
      );
    });

    it('should reject empty owner', () => {
      expect(() => GitHubRepository.create({ ...validProps, owner: '' })).toThrow(
        'Repository owner is required',
      );
    });

    it('should reject empty default branch', () => {
      expect(() => GitHubRepository.create({ ...validProps, defaultBranch: '' })).toThrow(
        'Default branch is required',
      );
    });

    it('should reject invalid URL', () => {
      expect(() => GitHubRepository.create({ ...validProps, url: 'not-a-url' })).toThrow(
    ...
    it('should reject mismatched fullName format', () => {
      expect(() => GitHubRepository.create({ ...validProps, fullName: 'invalid' })).toThrow(
        'Full name must be in format "owner/name"',
      );

      expect(() => GitHubRepository.create({ ...validProps, fullName: 'wrong/name' })).toThrow(
        'Full name must be in format "owner/name"',
      );

      expect(() => GitHubRepository.create({ ...validProps, fullName: 'octocat/wrong' })).toThrow(
        'Full name must be in format "owner/name"',
      );
    });
  });
